...
# 
# __Tip:__ Når vi angiver tekststrenge på et plot, kan vi skrive `r"$MATEMATIK$"` for at skrive udtryk ved hjælp af Latex. `r` betyder, at tekststrengen skal læses rå, så man ikke eksempelvis laver et linje skift, når man skriver `\n`, som python ellers ville gøre. 
...
# __Tip:__ Bemærk her, at vi kan foretage linjeskift efter `,` i et funktionskald for at gøre koden mere overskuelig.  
...
# Og for at ændre de enkelte plot, betragter vi det som en liste. Så for at ændre det i'te plot (husk nulindeksering), skriver vi: 
# ```python
...
# ## Gaffelfunktioner 
# Hvis vi ønsker at lave stykvis definerede funktioner, der har forskellige funktionsudtryk i forskellige intervaller (også kendt som "gaffelfunktioner"), kan vi benytte <code>sp.Piecewise()</code>. Kaldesekvenser er:
# ```python
...
# <code>sp.Piecewise</code> kan arbejde med et større antal udtryk og intervalbetingelser end blot to som i eksemplet. Hvis nogle $x$-værdier opfylder flere af intervalbetingelserne, anvender SymPy det første udtryk hvor et givet $x$ opfylder betingelsen. Man kan bruge dette på en smart måde, hvis man skal tegne en gaffelfunktion, hvor samme udtryk gælder i flere intervaller for $x$. Man kan eksempelvis lade det andet udtryk gælde alle steder, hvor den første ikke gælder, ved blot at angive andet udtryks intervalbetingelse som <code>True</code> og angive det som det sidste udtryk. Et eksempel på dette kunne være at betragte gaffelfunktionen:
# 
...
# 
# Plotter vi nu de to figurer (her sepereret med en konstant for at kunne se dem begge får vi:
...
# Dette giver altså samme resultat. 
